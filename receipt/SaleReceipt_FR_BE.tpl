			...
			{% if Sale.Shop.ReceiptSetup.creditcardAgree|strlen > 0 and not parameters.gift_receipt and not parameters.email %}
				{% for SalePayment in Sale.SalePayments.SalePayment %}
					{% if parameters.force_cc_agree or parameters.print_workorder_agree %}
						{{ _self.store_receipt(Sale,parameters,_context) }}
...
{% macro no_tax_applied_text(Sale) %}
	...
	{% if(has_non_taxed_item == 'true') %}
		<p class="thankyou">* Aucune taxe appliqu&eacute;e</p>
	{% endif %}
{% endmacro %}
	...
	{% if Line.Serialized %}
		{% for Serialized in Line.Serialized.Serialized %}
			<div class='line_serial'>
				Num&eacute;ro de s&eacute;rie : {{ Serialized.serial }} {{ Serialized.color }} {{ Serialized.size }}
			</div>
		{% endfor %}
	{% endif %}
	{% if options.per_line_employee %}
		<div class='line_note'>
			Employ&eacute;(e) : {{ Line.Employee.firstName }}
		</div>
	{% endif %}
{% endmacro %}
...
{% macro sale_details(Sale,options) %}
	<p id="receiptInfo" class="details">
		...
		{% if isVATAndRegistrationNumberOnReceipt() %}
			# TVA : {{Sale.Shop.vatNumber}}<br />
			# d'enregistrement cie : {{Sale.Shop.companyRegistrationNumber}}<br />
		{% endif %}

		{% if Sale.Register %}
			Caisse : <span id="receiptRegisterName">{{Sale.Register.name}}</span><br />
		{% endif %}

		{% if Sale.Employee %}
			Employ&eacute;(e) : <span id="receiptEmployeeName">{{Sale.Employee.firstName}}</span><br />
		{% endif %}
		...
		{% if Sale.Customer %}
			...
			{% if not options.show_customer_name_only %}
				...
				{% if options.show_full_customer_address and ContactAddress.address1 %}
					Adresse: {{ ContactAddress.address1 }}{% if ContactAddress.city %}, {{ ContactAddress.city }}{% endif %}{% if ContactAddress.state %}, {{ ContactAddress.state }}{% endif %}{% if ContactAddress.zip %}, {{ ContactAddress.zip }}{% endif %}<br />
				{% endif %}
				...
				<span id="receiptPhonesContainer" class="indent">
					...
					{% for Email in Sale.Customer.Contact.Emails.ContactEmail %}
						Courriel: <span id="receiptEmail">{{Email.address}} ({{Email.useType}})</span><br />
					{% endfor %}
				</span>
			{% endif %}
		...
		<th data-automation="lineItemDescription" class="description">
			...
			{% if options.per_line_discount == true and not parameters.gift_receipt %}
				{% if Line.calcLineDiscount > 0 %}
					<small>R&eacute;duction : '{{ Line.Discount.name }}' -{{Line.calcLineDiscount|money}}</small>
				{% elseif Line.calcLineDiscount < 0 %}
				       <small>R&eacute;duction : '{{ Line.Discount.name }}' {{Line.calcLineDiscount|getinverse|money}}</small>
			...
			<table class="totals">
				<tbody id="receiptSaleTotals">
					<tr>
						<td width="100%">
							{% if options.discounted_line_items and Sale.calcDiscount != 0 %}
								Sous-total avec r&eacute;ductions
							{% else %}
								Sous-total
							{% endif %}
						</td>
					...
					{% if not options.discounted_line_items and Sale.calcDiscount > 0 %}
						<tr><td>R&eacute;ductions</td><td id="receiptSaleTotalsDiscounts" class="amount">-{{Sale.calcDiscount|money}}</td></tr>
					{% elseif not options.discounted_line_items and Sale.calcDiscount < 0 %}
						<tr><td>R&eacute;ductions</td><td id="receiptSaleTotalsDiscounts" class="amount">{{Sale.calcDiscount|getinverse|money}}</td></tr>
					{% endif %}
					{% for Tax in Sale.TaxClassTotals.Tax %}
						{% if Tax.taxname and Tax.rate > 0 %}
							<tr><td data-automation="receiptSaleTotalsTaxName" width="100%">TVA: ({{Sale.calcTaxable|money}} @ {{Tax.rate}}%)</td><td data-automation="receiptSaleTotalsTaxValue" class="amount">{{Tax.amount|money}}</td></tr>
						{% endif %}
						{% if Tax.taxname2 and Tax.rate2 > 0 %}
							<tr><td data-automation="receiptSaleTotalsTaxName" width="100%">{{Tax.taxname2}} ({{Sale.calcTaxable|money}} @ {{Tax.rate2}}%)</td><td data-automation="receiptSaleTotalsTaxValue" class="amount">{{Tax.amount2|money}}</td></tr>						{% endif %}
					...
					{% for Payment in Sale.SalePayments.SalePayment %}
						{% if Payment.PaymentType.name != 'Cash' %}
							...
							{% if Payment.CreditAccount.giftCard == 'true' %}
								...
								{% if Payment.amount > 0 %}
									<tr>
										<td>Pr&eacute;l&egrave;vement sur carte cadeau</td>
			...
			{% if Sale.Customer.CreditAccount and Sale.Customer.CreditAccount.MetaData.creditBalanceOwed > 0 or Sale.Customer.CreditAccount.MetaData.extraDeposit > 0 %}
				...
				<table class="totals">
					...
					{% elseif Sale.Customer.CreditAccount.MetaData.extraDeposit > 0 %}
						<tr>
							<td width="100%">En d&eacute;p&ocirc;t : </td>
				...
				<td width="100%">
					...
					{% if Payment.CCCharge.MetaData.isEMV %}
						...
						{% if Payment.CCCharge.exp|strlen > 0 and
							...
							<br>Expiration de la carte : {{Payment.CCCharge.exp}}
						{% endif %}
...
{% macro workorders(Customer,parameters,options) %}
	{% if Customer.Workorders|length > 0 %}
		<h2>Demandes de service en cours</h2>
...
{% macro sale_cash_payment(Sale) %}
	...
	{% endfor %}
	{% if pay_cash == 'true' %}
		<tr><td width="100%">Comptant</td><td id="receiptPaymentsCash" class="amount">{{total|money}}</td></tr>
		<tr><td width="100%">Monnaie</td><td id="receiptPaymentsChange" class="amount">{{Sale.change|money}}</td></tr>
	{% endif %}
{% endmacro %}
