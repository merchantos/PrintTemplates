	...
	<p id="receiptInfo" class="details">
		...
		{% if isVATAndRegistrationNumberOnReceipt() %}
			{% if Sale.Shop.vatNumber|strlen %}
				VAT #: <span id="vatNumber">{{Sale.Shop.vatNumber}}</span><br />
			{% endif %}
			{% if Sale.Shop.companyRegistrationNumber|strlen %}
				Company registration #: <span id="companyRegistrationNumber">{{Sale.Shop.companyRegistrationNumber}}</span><br />
			{% endif %}
		{% endif %}
		...
		{% if Sale.Customer %}
			...
			{% if isVATAndRegistrationNumberOnReceipt() %}
				{% if Sale.Customer.vatNumber|strlen %}
					Customer VAT #: <span id="customerVatNumber">{{Sale.Customer.vatNumber}}</span><br />
				{% endif %}

				{% if Sale.Customer.companyRegistrationNumber|strlen %}
					Customer company registration #: <span id="customerCompanyRegistrationNumber">{{Sale.Customer.companyRegistrationNumber}}</span><br />
				{% endif %}
			{% endif %}
...
{% macro specialorders(Customer,isTaxInclusive,parameters,options) %}
	{% if Customer.SpecialOrders|length > 0 %}
		<h2 id="lineItemSectionSO">Special Orders</h2>
