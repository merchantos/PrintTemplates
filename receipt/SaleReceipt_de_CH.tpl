...
{% macro title(Sale,parameters,options) %}
	<h1 class="receiptTypeTitle">
		{# Check for preauth state first - preauth receipts show "Quittung" regardless of Sale.completed #}
		{% set preauth_state = Sale.MetaData.preauthState|default(parameters.preauth_state|default(null)) %}
		{% if preauth_state %}
			Quittung
		...
		<table class="sale lines">
			<tr>
				...
				{% if preauth_state == 'preauthorized' %}
					<th class="description">Vorautorisierung</th>
				{% elseif preauth_state == 'cancelled' %}
					<th class="description">Vorautorisierung - STORNIERT</th>
				{% elseif preauth_state == 'expired' %}
					<th class="description">Vorautorisierung - ABGELAUFEN</th>
				{% else %}
					{# captured or no preauth state - show default Artikel #}
			...
			<table class="saletotals totals">
				<tbody id="receiptSaleTotals">
					...
					{% if Sale.calcSurcharges != 0 %}
						<tr class="surcharge"><td>Zuschlag</td><td id="receiptSaleTotalsSurcharge" class="amount">{{Sale.calcSurcharges|money}}</td></tr>
					{% endif %}
				</tbody>
			</table>
		{% endif %}
					...
					{% for Payment in Sale.SalePayments.SalePayment %}
						{% if Payment.PaymentType.name != 'Cash' %}
							...
							{% if Payment.CreditAccount.giftCard == 'true' %}
								...
								{% if Payment.amount > 0 %}
									<tr>
										<td class="label">Geschenkgutschein-Aufladung {{Payment.CreditAccount.code}}</td>
							...
							{% elseif Payment.CreditAccount and Payment.archived == 'false' %}
								...
								{% if not surcharge_rendered and Payment.amount < 0 and not Sale.SaleLines and Sale.calcSurcharges != 0 %}
									<tr class="surcharge"><td class="label" width="100%">Zuschlag</td><td id="receiptSaleTotalsSurcharge" class="amount">{{Sale.calcSurcharges|money}}</td></tr>
...
{% macro remoteDepositSummary(Sale,parameters) %}
	{% if Sale.Customer.CreditAccount.MetaData.remoteDepositAmount %}
		{% if Sale.SaleLines is not empty %}
			...
			{{ _self.date(Sale) }}
		{% endif %}
		<h2 class="footerSectionTitle">Angeforderte Zahlung</h2>
		<table class="totals">
			<tbody>
				<tr>
					<td width="100%">Kontoeinzahlung: </td>
	...
	{% if Sale.Customer.CreditAccount.MetaData.creditBalanceOwedAfterPayment > 0 or Sale.Customer.CreditAccount.MetaData.extraDepositAfterPayment > 0 %}
		<h2 class="footerSectionTitle">Shop-Konto (nach Zahlung)</h2>
		<table class="totals">
			<tbody>
			{% if Sale.Customer.CreditAccount.MetaData.creditBalanceOwedAfterPayment > 0 %}
				<tr>
					<td width="100%">Geschuldeter Restbetrag (nach Zahlung): </td>
			...
			{% elseif Sale.Customer.CreditAccount.MetaData.extraDepositAfterPayment > 0 %}
				<tr>
					<td width="100%">Bei Einzahlung (nach Zahlung): </td>
	...
	{% if Sale.Customer.MetaData.getAmountToCompleteAllAfterPayment > 0 %}
		<table class="totals">
			<tr class="total">
				<td width="100%">Restliches Guthaben (nach Zahlung): </td>
...
{% macro layaways(Customer,isTaxInclusive,parameters,options) %}
	{% if Customer.Layaways and Customer.Layaways|length > 0 %}
		...
		<table class="layways totals">
			...
			<tr class="total">
				<td class="label" width="100%">Gesamt</td>
...
{% macro sale_cash_payment(Sale) %}
	...
	{% endfor %}
	{% if pay_cash == 'true' %}
		<tr><td class="label">Bar bezahlter Betrag</td><td id="receiptPaymentsCash" class="amount">{{total|money}}</td></tr>
        {% if Sale.MetaData.isCashRoundingEnabled|CompBool == true %}
        {% set cash_rounding_delta = Sale.MetaData.cash_rounding_delta|default(0) %}
        <tr><td class="label">Rundungsbetrag</td><td id="receiptPaymentsCashRounding" class="amount">{{cash_rounding_delta|money}}</td></tr>
        {% endif %}
