	...
	<p id="receiptInfo" class="details">
		...
		{% if isVATAndRegistrationNumberOnReceipt() %}
			{% if Sale.Shop.vatNumber|strlen %}
				N.º IVA: <span id="vatNumber">{{Sale.Shop.vatNumber}}</span><br />
			{% endif %}
			{% if Sale.Shop.companyRegistrationNumber|strlen %}
				Número de registro: <span id="companyRegistrationNumber">{{Sale.Shop.companyRegistrationNumber}}</span><br />
			{% endif %}
		{% endif %}

		{% if Sale.Register %}
			Registro: <span id="receiptRegisterName">{{Sale.Register.name}}</span><br />
		{% endif %}

		{% if Sale.Employee %}
			Empleado: <span id="receiptEmployeeName">{{Sale.Employee.firstName}}</span><br />
		{% endif %}

		{% if Sale.Customer %}
			{% if Sale.Customer.company|strlen > 0 %}
				Companía: <span id="receiptCompanyName">{{Sale.Customer.company}}</span><br />
			{% endif %}

			{% if not options.company_name_override or not Sale.Customer.company|strlen > 0 %}
				Cliente: <span id="receiptCustomerName">{{Sale.Customer.firstName}} {{Sale.Customer.lastName}}</span><br />
			{% endif %}
			...
			{% if not options.show_customer_name_only %}
				...
				{% if options.show_full_customer_address and ContactAddress.address1 %}
					Dirección: {{ ContactAddress.address1 }}{% if ContactAddress.city %}, {{ ContactAddress.city }}{% endif %}{% if ContactAddress.state %}, {{ ContactAddress.state }}{% endif %}{% if ContactAddress.zip %}, {{ ContactAddress.zip }}{% endif %}<br />
				{% endif %}
			...
			{% if isVATAndRegistrationNumberOnReceipt() %}
				{% if Sale.Customer.vatNumber|strlen %}
					N.º IVA cliente:: <span id="customerVatNumber">{{Sale.Customer.vatNumber}}</span><br />
				{% endif %}

				{% if Sale.Customer.companyRegistrationNumber|strlen %}
					Número de registro cliente: <span id="customerCompanyRegistrationNumber">{{Sale.Customer.companyRegistrationNumber}}</span><br />
				{% endif %}
			{% endif %}

			{% if options.show_customer_notes %}
				{% if Sale.Customer.Note.note|strlen > 0 %}
					Nota: {{ Sale.Customer.Note.note|noteformat|raw }}<br />
				{% endif %}
			{% endif %}
		{% endif %}
	</p>
			...
			<thead>
				<tr>
					...
					{% if options.show_custom_sku and options.show_manufacturer_sku %}
						...
						<th class="custom_field">MSKU ger.</th>
						...
						{% if Payment.PaymentType.name != 'Cash' %}
							...
							{% if Payment.CreditAccount.giftCard == 'true' %}
								...
								{% elseif Payment.amount < 0 and Sale.calcTotal < 0 %}
									<tr>
										<td>Reembolso en trajeta regalo</td>
								...
								{% elseif Payment.amount < 0 and Sale.calcTotal >= 0 %}
									<tr>
										<td>Adquisición tarjeta regalo</td>
										<td id="receiptPaymentsGiftCardValue" class="amount">{{Payment.amount|getinverse|money}}</td>
									</tr>
									<tr>
										<td>Saldo</td>
...
{% macro specialorders(Customer,isTaxInclusive,parameters,options) %}
	{% if Customer.SpecialOrders|length > 0 %}
		<h2 id="lineItemSectionSO">Pedidos especiales</h2>
...
{% macro sale_cash_payment(Sale) %}
	...
	{% endfor %}
	{% if pay_cash == 'true' %}
		<tr><td width="100%">Efectivo</td><td id="receiptPaymentsCash" class="amount">{{total|money}}</td></tr>
		<tr><td width="100%">Cambio</td><td id="receiptPaymentsChange" class="amount">{{Sale.change|money}}</td></tr>
	{% endif %}
{% endmacro %}
