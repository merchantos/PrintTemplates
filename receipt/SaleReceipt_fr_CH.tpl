		...
		{% if isVATAndRegistrationNumberOnReceipt() %}
			...
			{% if Sale.Shop.companyRegistrationNumber|strlen and not options.hide_shop_registration_number %}
				<span class="companyRegistrationNumberField">
					<span class="companyRegistrationNumberLabel"># d'enregistrement de l'entreprise: </span>
		...
		{% if Sale.Customer %}
			...
			{% if isVATAndRegistrationNumberOnReceipt() %}
				...
				{% if Sale.Customer.companyRegistrationNumber|strlen and not options.hide_customer_registration_number %}
					<span class="receiptCustomerCompanyVatField">
						<span class="receiptCustomerCompanyVatLabel"># d'enregistrement de l'entreprise cliente: </span>
			...
			{% if options.show_customer_notes %}
				{% if Sale.Customer.Note.note|strlen > 0 %}
					<span class="receiptCustomerNoteField"><span class="receiptCustomerNoteLabel">Remarque: </span>{{ Sale.Customer.Note.note|noteformat|raw }}<br /></span>
				{% endif %}
			{% endif %}
		{% endif %}
	</p>
			...
			<table class="saletotals totals">
				<tbody id="receiptSaleTotals">
					...
					{% if Sale.calcSurcharges != 0 %}
						<tr class="surcharge"><td>Supplément</td><td id="receiptSaleTotalsSurcharge" class="amount">{{Sale.calcSurcharges|money}}</td></tr>
					{% endif %}
				</tbody>
			</table>
		{% endif %}
						...
						{% if Payment.PaymentType.name != 'Cash' %}
							...
							{% elseif Payment.CreditAccount and Payment.archived == 'false' %}
								...
								{% if not surcharge_rendered and Payment.amount < 0 and not Sale.SaleLines and Sale.calcSurcharges != 0 %}
									<tr class="surcharge"><td class="label" width="100%">Supplément</td><td id="receiptSaleTotalsSurcharge" class="amount">{{Sale.calcSurcharges|money}}</td></tr>
				...
				<td class="label" width="100%">
					...
					{% if Payment.CCCharge.MetaData.isEMV %}
						{% if Payment.CCCharge.MetaData.AID|strlen > 0 %}
							<br>ID de demande : {{Payment.CCCharge.MetaData.AID}}
						{% endif %}
...
{% macro sale_cash_payment(Sale) %}
	...
	{% endfor %}
	{% if pay_cash == 'true' %}
		<tr><td class="label">Montant payé en espèces</td><td id="receiptPaymentsCash" class="amount">{{total|money}}</td></tr>
        {% if Sale.MetaData.isCashRoundingEnabled|CompBool == true %}
            {% set cash_rounding_delta = Sale.MetaData.cash_rounding_delta|default(0) %}
            <tr><td class="label">Arrondi</td><td id="receiptPaymentsCashRounding" class="amount">{{cash_rounding_delta|money}}</td></tr>
        {% endif %}
