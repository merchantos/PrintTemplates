	...
	{% for Workorder in Workorders %}
		<div class="workorder {% if not loop.last %} pagebreak{% endif %}">
			...
			<div class="header">
				...
				{% if parameters.type == 'shop-tag' %}
					{% if Workorder.hookIn|strlen > 0 or Workorder.hookOut|strlen > 0 %}
						<h1 style="margin-top:20px;">Hook Ein: {{Workorder.hookIn}} <br />
						Hook Aus: {{Workorder.hookOut}}</h1>
					{% endif %}
				{% endif %}
			</div>
			...
			<div class="detail">
				...
				{% for serializedID in Workorder.Serialized %}
					{% if hide_work_order_item == false %}
						<div class="workOrderItem" >Auftragsposition::</div>
				...
				{% if hide_start_and_date == false %}
					Gestartet: {{Workorder.timeIn|correcttimezone|date ("m/d/y h:i a")}}<br />
					Auftragsende: {{Workorder.etaOut|correcttimezone|date ("m/d/y h:i a")}}<br />
					{% else %}
				{% endif %}
				{% if hide_employee == false %}
					Mitarbeiter/in: {{ Workorder.Employee.firstName }} {{ Workorder.Employee.lastName }}
				{% else %}
				{% endif %}
				</h2>
			</div>
			...
			<table class="lines">
				...
				<tr>
					<th class="description">Position/Tätigkeit</th>
				...
				{% if specialorder == true %}
					<tr>
						<th class="specialorder">Sonderbestellungen</th>
						<th></th>
						<th></th>
					</tr>
					{% for WorkorderItem in Workorder.WorkorderItems.WorkorderItem %}
...
{% macro line(Line,parameters,options) %}
	<tr data-automation="lineItemRow">
		<td data-automation="lineItemRowItemLabor" class="description">
			...
			{% if Line.Discount %}
				<small>Rabatt: '{{ Line.Discount.name }}' (-{{ Line.SaleLine.calcLineDiscount|money }})</small>
			{% endif %}
		</td>
		{% if options.per_line_subtotal == true %}
			{% if options.itemized_hours_labor == true %}
				...
				{% elseif Line.hours > 0 or Line.minutes > 0 %}
					<td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} Stunden {{ Line.minutes }} Minuten x {{ options.servicerate|money }}/Stunden</td>
		...
		{% elseif options.itemized_hours_labor == true and Line.unitPriceOverride == '0'%}
			{% if Line.hours > 0 or Line.minutes > 0 %}
				<td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} Stunden {{ Line.minutes }} Minuten</td>
			{% else %}
