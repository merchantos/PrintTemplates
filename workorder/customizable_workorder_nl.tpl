	...
	{% for Workorder in Workorders %}
		<div class="workorder {% if not loop.last %} pagebreak{% endif %}">
			...
			<div class="header">
				...
				{% endif %}
				{% if minimal_mode == true %}
					<h1 id="receiptTypeTitle">Werkordernr.{{ Workorder.workorderID }}</h1>
				...
				{% else %}
						...
						{% elseif make_work_order_number_small == false %}	
							<h1 id="receiptTypeTitle">Werkorder</h1>
							{{ _self.date(Workorder) }}
							<br />
				...
				{% if parameters.type == 'shop-tag' %}
					{% if Workorder.hookIn|strlen > 0 or Workorder.hookOut|strlen > 0 %}
						<h1 style="margin-top:20px;">In: {{Workorder.hookIn}} <br />
						Uit: {{Workorder.hookOut}}</h1>
					{% endif %}
				{% endif %}
			</div>
			...
			<div class="detail">
				...
				{% for serializedID in Workorder.Serialized %}
					{% if hide_work_order_item == false %}
						<div class="workOrderItem" >Werkorderitem::</div>
				...
				{% if hide_start_and_date == false %}
					Gestart: {{Workorder.timeIn|correcttimezone|date ("m/d/y h:i a")}}<br />
					Deadline: {{Workorder.etaOut|correcttimezone|date ("m/d/y h:i a")}}<br />
					{% else %}
				{% endif %}
				{% if hide_employee == false %}
					Werknemer: {{ Workorder.Employee.firstName }} {{ Workorder.Employee.lastName }}
				{% else %}
				{% endif %}
				</h2>
			</div>
			...
			<table class="lines">
				...
				<tr>
					<th class="description">Item/arbeid</th>
				...
				{% if specialorder == true %}
					<tr>
						<th class="specialorder">Speciale bestellingen</th>
						<th></th>
						<th></th>
					</tr>
					{% for WorkorderItem in Workorder.WorkorderItems.WorkorderItem %}
...
{% macro line(Line,parameters,options) %}
	<tr data-automation="lineItemRow">
		<td data-automation="lineItemRowItemLabor" class="description">
			...
			{% if Line.Discount %}
				<small>Korting: '{{ Line.Discount.name }}' (-{{ Line.SaleLine.calcLineDiscount|money }})</small>
			{% endif %}
		</td>
		{% if options.per_line_subtotal == true %}
			{% if options.itemized_hours_labor == true %}
				...
				{% elseif Line.hours > 0 or Line.minutes > 0 %}
					<td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} uren {{ Line.minutes }} minuten x {{ options.servicerate|money }}/uren</td>
		...
		{% elseif options.itemized_hours_labor == true and Line.unitPriceOverride == '0'%}
			{% if Line.hours > 0 or Line.minutes > 0 %}
				<td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} uren {{ Line.minutes }} minuten</td>
			{% else %}
