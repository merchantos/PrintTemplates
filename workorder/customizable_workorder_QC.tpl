    ...
    {% for Workorder in Workorders %}
        <div class="workorder {% if not loop.last %} pagebreak{% endif %}">
            ...
            <div class="header">
                ...
                {% if parameters.type == 'shop-tag' %}
                    {% if Workorder.hookIn|strlen > 0 or Workorder.hookOut|strlen > 0 %}
                        <h1 style="margin-top:20px;">Crochet entrant: {{Workorder.hookIn}} <br />
                        Crochet sortant: {{Workorder.hookOut}}</h1>
                    {% endif %}
                {% endif %}
            </div>
            ...
            <div class="detail">
                ...
                {% for serializedID in Workorder.Serialized %}
                    {% if hide_work_order_item == false %}
                        <div class="workOrderItem"><h3>Item&#40;s&#41;:</h3></div>
                ...
                {% if hide_status == false %}
                    Statut: {{ Workorder.WorkorderStatus.name }}<br />
                {% endif %}
                ...
                {% if hide_start_and_date == false %}
                    D&eacute;but: {{Workorder.timeIn|correcttimezone|date ("m/d/y h:i a")}}<br />
                    D&ucirc; le: {{Workorder.etaOut|correcttimezone|date ("m/d/y h:i a")}}<br />
                    {% else %}
                {% endif %}
                {% if hide_employee == false %}
                    Employ&eacute;&#40;e&#41;: {{ Workorder.Employee.firstName }} {{ Workorder.Employee.lastName }}
                    {% else %}
                {% endif %}
                </h2>
            </div>
            ...
            <table class="lines">
                ...
                <tr>
                    <th class="description">Item/Main d&#39;oeuvre</th>
                ...
                {% if specialorder == true %}
                    <tr>
                        <th class="specialorder">Commande&#40;s&#41; sp&eacute;ciale&#40;s&#41;</th>
                        <th></th>
                        <th></th>
                    </tr>
                    {% for WorkorderItem in Workorder.WorkorderItems.WorkorderItem %}
...
{% macro line(Line,parameters,options) %}
    <tr data-automation="lineItemRow">
        <td data-automation="lineItemRowItemLabor" class="description">
            ...
            {% if Line.Discount %}
                <small>R&eacute;duction: '{{ Line.Discount.name }}' (-{{ Line.SaleLine.calcLineDiscount|money }})</small>
            {% endif %}
        </td>
        {% if options.per_line_subtotal == true %}
            {% if options.itemized_hours_labor == true %}
                ...
                {% elseif Line.hours > 0 or Line.minutes > 0 %}
                    <td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} heure&#40;s&#41; {{ Line.minutes }} minute&#40;s&#41; x {{ options.servicerate|money }}/heure</td>
        ...
        {% elseif options.itemized_hours_labor == true and Line.unitPriceOverride == '0'%}
            {% if Line.hours > 0 or Line.minutes > 0 %}
                <td data-automation="lineItemQuantity" class="quantity">{{ Line.hours }} heure&#40;s&#41; {{ Line.minutes }} minute&#40;s&#41;</td>
            {% else %}
    ...
    {% if Line.itemID != 0 %}
        ...
        {% if Line.isSpecialOrder == 'true' %}
            <div data-automation="lineItemRowEmployeeName" class='line_note'>
                Employ&eacute;&#40;e&#41;: {{ Line.Employee.firstName }} {{ Line.Employee.lastName }}
            </div>
        {% endif %}
    {% else %}
